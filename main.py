...
class AppView:
    ...
    def build_ui(self):
        ...
        self.auto_radio = tk.Radiobutton(
            self.options_frame,
            text="Auto Threads",
            variable=self.mode_var,
            value="auto",
    ...
    def toggle_thread_mode(self):
        ...
        if mode == "auto":
            ...
            self.thread_label.config(
                text=f"Workers: AUTO ({multiprocessing.cpu_count()})"
            )
        else:
...
class Controller:
    ...
    def process(self):
        ...
        strategy = (
            ManualThreadStrategy(self.view.slider.get())
            if mode == "manual"
            else AutoThreadStrategy()
        )
    ...
    def update_ui(
        ...
        self.view.result_text.config(text=result)
